...
public class SerialParser implements SerialPortEventListener{
	...
	private void handleData(short[] message){
		short tag = message[0];
		float data = ((float)(message[1]<<24 | message[2]<<16 | message[3]<<8 | message[4]))/100000;
		parent.updateData(tag, data);
	}

	private void handleWaypoint(short[] message){
		short tag = message[0];
		float latitude  = ((float)(message[1]<<24 | message[2]<<16 | message[3]<<8 | message[4]))/100000;
		float longitude = ((float)(message[5]<<24 | message[6]<<16 | message[7]<<8 | message[8]))/100000;
